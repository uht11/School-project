...
//printRang method
  public void printRang(BSTNode<Key,E> rt, Key k, Key l) {
    if (rt == null) return;
    printRang(rt.left(),k,l);
    if (rt.key().compareTo(k)>= 0 && rt.key().compareTo(l)<= 0)
    {
      printVisit(rt.element());
      System.out.print(rt.element()+",");
     }
      printRang(rt.right(),k,l);
  }
...
//PostOrderMethod
private void printpostOrder(BSTNode<Key,E> rt) {
    if (rt == null) return;
...
//PreOrderMethod
private void printpreOrder(BSTNode<Key,E> rt) {
    if (rt == null) return;
    printVisit(rt.element());
